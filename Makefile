...
PROTO_ROOT = pkg/session_manager_service
PROTO_FILE = session_manager.proto
GEN_DIR = pkg/gen/session_manager_service
GO_MODULE = github.com/psds-microservice/session-manager-service
OPENAPI_OUT = api
.DEFAULT_GOAL := help
...
proto-generate-local:
	@mkdir -p $(GEN_DIR) $(OPENAPI_OUT); command -v protoc-gen-grpc-gateway >/dev/null 2>&1 || (echo "Install: go install github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-gateway@latest" && exit 1); PATH="$$(go env GOPATH)/bin:$$PATH"; protoc -I $(PROTO_ROOT) -I third_party --go_out=. --go_opt=module=$(GO_MODULE) --go-grpc_out=. --go-grpc_opt=module=$(GO_MODULE) --grpc-gateway_out=. --grpc-gateway_opt=module=$(GO_MODULE) $(PROTO_ROOT)/$(PROTO_FILE); echo "OK: $(GEN_DIR)"
